...
math: true
image:
  path: /assets/img/posts/cs231n/rnn/types.png
  alt: "Figure 1. Different (non-exhaustive) types of Recurrent Neural Network architectures."
---
...
![Figure 2. Simplified RNN box (Left) and Unrolled RNN (Right).](/assets/img/posts/cs231n/rnn/rnn_blackbox.png){: width="398" height="752" }
![Figure 2. Simplified RNN box (Left) and Unrolled RNN (Right).](/assets/img/posts/cs231n/rnn/unrolledRNN.png){: width="1968" height="1216" }
_**Figure 2.**Simplified RNN box (Left) and Unrolled RNN (Right)._
...
<!-- markdownlint-capture -->
<!-- markdownlint-disable -->
> **역주.** $$g_t$$가 "gate" 게이트라는 다소 헷갈리는 이름으로 불리지만, 정작 시그모이드가 아니라 tanh를 쓴다는 점에서 $$f, i, o$$와는 성격이 다르다. 다른 자료에서는 흔히 $$g_t$$를 후보 셀 상태(candidate cell state, 흔히 $$\tilde{c}_t$$로 표기)라고 부른다. 실제로 값을 0과 1 사이로 열고 닫는 진짜 게이트는 $$f, i, o$$ 셋뿐이고, $$g_t$$는 그 게이트들이 걸러낼 "새로 셀에 넣을 후보 값"을 만드는 역할이다.
{: .prompt-tip }
<!-- markdownlint-restore -->
...
$$f, i, o$$ 게이트 벡터의 값은 모두 시그모이드 함수 $$\sigma$$로 눌러 넣어졌기 때문에 0에서 1 사이의 범위를 가지며, 이 값들을 원소별로 곱하면 다음을 알 수 있다.
...
- **Input 게이트:** 스텝 $$t$$의 input 게이트 $$i_t$$는 이전 은닉 상태 $$h_{t-1}$$과 입력 $$x_t$$로부터 다음 셀 상태 $$c_t$$에 얼마만큼의 정보를 "추가"해야 하는지 조절한다. tanh 대신 "input" 게이트 $$i$$는 시그모이드 함수를 가지는데, 이는 입력을 0과 1 사이의 값으로 바꾼다. 이는 값이 거의 언제나 0이거나 거의 언제나 1인 스위치 역할을 한다. 이 "input" 게이트는 "gate" 게이트 $$g$$가 만들어내는 RNN 출력을 취할지 말지를 결정하고, 그 출력을 input 게이트 $$i$$와 곱한다.
